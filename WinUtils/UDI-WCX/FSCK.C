	...
	char *argv[];
{
	...
	if (argc != 1) {
		fprintf(stderr,"usage: fsck [-y] [partN:]d:\\path\\DriveImageFileName.ext\n");
		xexit(-1);
	}
        for (dev=0; dev<MAXDRIV; dev++) *DriveImage[dev]=0;
	dev = 0;                                /* ((argv[ap+0][0] & 223) - 65); */
        strncpy(DriveImage[dev], p, MAXPATH);
        if (! File_Exists(DriveImage[dev])) {
                PF("\nFile `%s` not exists.", DriveImage[dev]); xexit(-1);
        }
#ifdef _MSX_DOS
	if (!_yes) {
	...
	if (filsys.s_mounted != SMOUNTED) {
		PF("Device `%s` has invalid magic number %d. Fix? ",
		       DriveImage[dev], filsys.s_mounted);
		if (!yes())
			xexit(-1);
		filsys.s_mounted = SMOUNTED;
		dwrite(SUPERBLOCK, &filsys);
	}
	PF("Checking drive `%s` with fsize %u blocks, isize %u blocks, rsize %u blocks. Confirm? ",
	       DriveImage[dev], filsys.s_fsize, filsys.s_isize,
