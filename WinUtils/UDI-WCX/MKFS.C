...
void usage() {
 	...
 	fprintf(stderr, "usage: mkfs [-yfqv] [partN:]d:\\path\\DriveImageFileName.ext fsize isize [rsize]\n");
 	xexit(-1);
}
...
void main(argc, argv)
	...
	char *argv[];
{
	uint fsize=0, isize=0, rsize=0, ap=0;
	uchar *s;
        int fe;
        FILE* fl;
	...
	initenv();
#endif
        for (fe=0; fe<MAXDRIV; fe++) *DriveImage[fe]=0;
	dev = 0;                                /* ((argv[ap+0][0] & 223) - 65); */
	...
	for (++argv; --argc > 0; ) {
		...
		if (*s == '-') {
			...
			default:
                ...
                else if (!DriveImage[dev][0])
                        strncpy(DriveImage[dev], s, MAXPATH);
                else if (! fsize)
                	fsize = (uint) atoi(s);
                else if (! isize)
                	isize = (uint) atoi(s);
        ...
        if ((!fsize)||(!isize)) usage();
        if (fe=File_Exists(DriveImage[dev])) {
                if  (!yes("\nFile exists. Overwrite?(Y/N): ")) xexit(-1);
                ap=1;
        }
	...
	PF("\nMaking filesystem on `%s`:\n  fsize %u blocks (%u kb), isize %u blocks (%u inodes), rsize %u.",
	       DriveImage[dev], fsize, fsize*SEC_SIZE/1024, isize, isize*DINODESPERBLOCK, rsize); FF;
	if (!yes("\nConfirm?(Y/N): "))
		xexit(-1);
#ifdef _MSX_DOS
	if (!_yes) {
...
#endif
        if ((!fe)|| ap) {
                s = (uchar *) malloc(SEC_SIZE);   /* allocate memory */
                memset(s, 0, SEC_SIZE);
                if (!s) { PF("\nError: malloc\n"); xexit(-1); }
                if (! (fl = fopen(DriveImage[dev], "wb") )) {  PF("\nError: fopen\n"); xexit(-1); }
                for (fe=fsize;fe;fe--)
                   if (fwrite(s, SEC_SIZE, 1, fl)!=1) {  PF("\nError: fwrite\n"); xexit(-1); }
                fclose(fl);
                free(s);   /* free memory */
        }
	PF("\n");
