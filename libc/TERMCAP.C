...
static int scan_file __P((char *str, int fd, pbuffer bufp));
static int name_match __P((char *line, char *name));
static int compare_contin __P((char *str1, char *str2));
static char *gobble_line __P((int fd, pbuffer bufp, char *append_end));
...
void tputs(char *str, int nlines, outfun_t outfun)
{
 ...
 */
static int scan_file(char *str, int fd, pbuffer bufp)
{
	register char *end;
 ...
 */
static char *gobble_line(int fd, pbuffer bufp, char *append_end)
{
	int nread;
	...
	while (1) {
		...
		if (bufp->ptr == buf) {
			...
			nread = bufp->ptr - buf;		/* 2015 */
			append_end = append_end - nread; 
			bufp->full = bufp->full - nread;
			bcopy(bufp->ptr, buf, bufp->full);	/* 2015 */
			bufp->ptr = buf;
		}
