...
/* extern void bcopy(void *src, void *dst, uint count);	/* NEED__MACHDEP */
...
GBL int sys_stime(VOID) {
	if (super()) {
#ifdef ORI_UZIX
		UDATA(u_argn1)=B_LDIRTO2(sizeof(time_t), tvec, (void*)TEMPDBUF);
#endif
		sttime(tvec);
		return 0;
	}
...
GBL int sys_times(VOID) {
	if (valadr(buf, sizeof(struct tms))) {
		_di();
#ifdef ORI_UZIX
		B_COPY(&UDATA(u_utime), buf, 4 * sizeof(time_t));
		B_LDIRTON(sizeof(time_t), &ticks, &buf->tms_etime);
#else		
...
#ifdef ORI_UZIX
	if ((uint)addr < PROGBASE || ((uint)addr + 256) >= fstack()) 
#else
...
GBL int sys_signal(VOID) {									/* http://man7.org/linux/man-pages/man2/signal.2.html */
