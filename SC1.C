...
#undef NEED__MACHDEP
#include "extern.h"
extern int Min(int a, int b);							/* NEED__MACHDEP */
extern int valadr(void *base, uint size);				/* NEED__MACHDEP */
extern int getfsys(dev_t devno, void *buf);				/* NEED__SCALL */
extern int swap_mmread(struct swap_mmread *rp);			/* NEED__DEVSWAP */
extern void B_LDIRTON(unsigned int count, void *src, void *dst);	/* send block from KERNEL_PAGE to process page */
extern int B_LDIRTO2(unsigned int count, void *src, void *dst);	/* send block from process page to KERNEL_PAGE, return (int)dst */
...
loop:		while (todo) {
			...
			if (write) {
				...
				if (bp == NULL) return;
#ifdef ORI_UZIX
				if ( *(((uchar*)&UBAS)+1) < 0xf0 )			/* if buffer in process local memory */
					B_LDIRTO2(amount, UBAS, bp + uoff);
				else										/* if buffer in shared memory */
#endif
...
#ifdef ORI_UZIX
#define buf ((struct utimbuf *)UDATA(u_argn3))
#else
#define buf ((struct utimbuf *)UDATA(u_argn2))
#endif
GBL int sys_utime(VOID) {
	struct utimbuf lcl, *bufp;
	register inoptr ino;

#ifdef ORI_UZIX
	_di();
	UDATA(u_argn1)=B_LDIRTO2(BUFSIZE/2, path, (void*)TEMPDBUF);
	UDATA(u_argn3)=B_LDIRTO2(sizeof(struct utimbuf), (void*)UDATA(u_argn2), (void*)TEMPDBUFX);
#endif
	bufp = buf;
...
#ifdef ORI_UZIX
	B_LDIRTON(sizeof(struct utimbuf), (void*)TEMPDBUFX, (void*)UDATA(u_argn2));
	_ei();
#endif
	ino->c_dirty=1;
	i_deref(ino);
...
#ifdef ORI_UZIX
	_di();
	UDATA(u_argn1)=B_LDIRTO2(BUFSIZE/2, name, (void*)TEMPDBUF);
#endif
...
#ifdef ORI_UZIX
	_di();
	UDATA(u_argn1)=B_LDIRTO2(BUFSIZE/2, dir, (void*)TEMPDBUF);
#endif
...
GBL int sys_chroot(VOID) {
	if (sys_chdir() == 0) {				/* ORION: DO NOT B_LDIRTO2 here - it is executing in sys_chdir()*/
...
GBL int sys_statfstat(VOID) {
	register inoptr ino;
#ifdef ORI_UZIX
	struct stat stst;			/* reserve on stack */
#endif
...
Err:		return -1;
	...
	_ei();
	stcpy(ino, &stst);
	B_LDIRTON(sizeof(struct stat), (void*)&stst, (void*)UDATA(u_argn2));
#else
...
GBL int sys_ioctl(VOID) {
	...
	if ((ino = getinode(fd)) == NULL)
		...
		UERR = EPERM;
	else if (!d_ioctl(DEVNUM(ino), request, data))	/* TODO: data not used in ioctls yet, so no transfer. Dont forget about transfer at time when it will be used */
		return 0;
	return -1;
}
#undef fd
...
#ifdef ORI_UZIX
	_di();
	UDATA(u_argn1)=B_LDIRTO2(BUFSIZE/2, spec, (void*)TEMPDBUF);
	UDATA(u_argn2)=B_LDIRTO2(BUFSIZE/2, dir, (void*)TEMPDBUFX);
#endif
	if (!super()) {
		UERR = EPERM;
		goto Err1;
...
#ifdef ORI_UZIX
	rdtime((void*)&UDATA(u_argn2));
	B_COPY(&UDATA(u_argn2),(void*)UDATA(u_argn1),sizeof(time_t));
#else
	rdtime(tvec);
#endif
	return 0;
...
#ifdef __KERNEL__
#define tmpbuf ((udata_t *)TEMPDBUFX)
...
#ifdef ORI_UZIX
#define xbuf ((uchar*)TEMPDBUF+sizeof(info_t))
#define ubuf ((struct s_pdata *)TEMPDBUF)
#define kbuf ((struct s_kdata *)TEMPDBUF)
#define ibuf ((info_t *)TEMPDBUF)
extern uint fstack(void);
#else
#define ubuf ((struct s_pdata *)buf)
#define kbuf ((struct s_kdata *)buf)
#define ibuf ((info_t *)buf)
#endif
	...
	void *buf;
{
	...
	uchar cnt = 1;
#ifdef	ORI_UZIX	
	int result, tocopy=sizeof(filesys_t);  /* 2014 */
	ptptr pp; /* 2015 */
	...
	if (dev == NULL) {
		switch (devno) {
		case GI_FTAB:	panic("GI_FTAB"); /* cnt = NFS;	ptr = fs_tab; */		break;		/* PANIC is temporary patch: */
		case GI_ITAB:	panic("GI_ITAB"); /* cnt = ITABSIZE; ptr = i_tab; */ 	break;		/* too much to copy :( */
		case GI_BTAB:	panic("GI_BTAB"); /* cnt = NBUFS;	ptr = bufpool; */	break;
#ifdef __KERNEL__
		case GI_PTAB:	cnt = PTABSIZE; 
						bcopy(ptab, pp=(void*)xbuf, tocopy=PTABSIZE*sizeof(ptab_t));
						result=fstack()-tocopy-(512+(int)ptab);					/* delta between ptab-to-ptab pointers and buf-in-process pointers */
						while (pp < ((ptptr)xbuf)+PTABSIZE) {
							pp->p_pptr=(void*)(((uchar*)pp->p_pptr)+result);	/* correct pointers before buf transfer to process area */
							pp->p_wait=((uchar*)pp->p_wait)+result;
							++pp;
						}
						ptr=xbuf;
						break;
		case GI_UDAT:	ptr = UDATA_ADDR; 	/* at F-area, no copy */	break;
		case GI_UTAB:	ptr = UDATA(u_ptab); tocopy=sizeof(ptab_t);	break;
		case GI_PDAT:	
#ifdef	ORI_UZIX	
						B_LDIRTO2(sizeof(struct s_pdata), buf, (void*)TEMPDBUF);
						result=pdat(ubuf); 											
						B_LDIRTON(sizeof(struct s_pdata), (void*)TEMPDBUF, buf);
						_ei();
#else
						result=pdat(ubuf); 											
#endif
...
#ifdef	ORI_UZIX	
						B_LDIRTON(sizeof(struct s_kdata), (void*)TEMPDBUF, buf);
						_ei();
#endif
				return 0;
#endif /* __KERNEL__ */
	...
	ibuf->ptr = ptr;
#ifdef	ORI_UZIX	
	if ((uint)ptr < 0xf000) {																/* if points to buffer in local kernel page */
		if ((uint)(ibuf->ptr=(uchar*)(fstack()-512-tocopy)) < (uint)UDATA(u_break)+256) { 	/* 256+256=512: proc_stack+trap_stack reserv, 256: heap reserv*/
			UERR = ENOMEM;
			return (-1);	
		}
		B_LDIRTON(tocopy, ptr, ibuf->ptr);										/* (uchar *)PROC_BUF; */
	}
	B_LDIRTON(sizeof(info_t), (void*)TEMPDBUF, buf);
	_ei();
#endif
	return 0;
}
