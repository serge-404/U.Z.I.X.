...
# endif
	...
	if(fin == NULL)
		error (cant_open,argc>1?argv[1]:"standard input");
	gch();
...
nxtpos = positions = myalloc(maxpos,sizeof(*positions));
	if(tmpstat == 0 || foll == 0 || positions == 0 ||
		...
		error(little_core, "state generation");
...
*/
	...
	if(verify == 0 || advance == 0 || stoff == 0)
		error(little_core, "final packing");
}
