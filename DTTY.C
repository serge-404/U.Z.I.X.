...
#include "devtty.msx"
#else
#include "devtty.orn"   /* ORION_HOSTED */
#endif
#endif
	...
	uchar rawflag;
{
	...
	while (nread < UDATA(u_count)) {
		for (;;) {
			...
			psleep(&ttyinq); /* wait for char */								/*  <<< HANG  */
...
NoKey:			return 0;		/* EOF */
		...
		if ( *(((uchar*)&UDATA(u_base))+1) < 0xf0 )	 {		/* if buffer in process local memory */
			B_LDIRTON(1, &ch, UDATA(u_base)++);
		}
		else										/* if buffer in shared memory */
#endif
		*(UDATA(u_base))++ = ch;

		if ((rawmode==3)||(!rawmode && ch == '\n'))
...
#ifdef ORI_UZIX
			if ( *(((uchar*)&UDATA(u_base))+1) < 0xf0 ) {	/* if buffer in process local memory */
				B_LDIRTO2(1, UDATA(u_base)++, &UDATA(u_argn4));
				_putc((uchar)UDATA(u_argn4));
			}
			else											/* if buffer in shared memory */
#endif
			_putc(*UDATA(u_base)++);
