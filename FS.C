...
static char *gtinobadoft_f = "getinode: bad oft %u for %u";
static char *gtinobadoft_i = "getinode: bad oft %u, inode %u";
	...
	register ino_t ino;
{
	...
	buf = (dinode_t *)bread(devno, inodeblock(dev, ino), 0);                      /*HERE*/
	...
	uchar uindex;
{
	...
	if (oftindex >= OFTSIZE)
		panic(gtinobadoft_f, oftindex, uindex);
	if ((inoindex = of_tab[oftindex].o_inode) < i_tab ||
	    inoindex >= i_tab + ITABSIZE)
		panic(gtinobadoft_i, oftindex, inoindex);
	magic(inoindex);
	return inoindex;
}
